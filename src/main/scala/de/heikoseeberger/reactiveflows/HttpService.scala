...
object HttpService {
  ...
  private[reactiveflows] def route(
    ...
    def flowEvents = path("flow-events") {
      ...
      get {
        complete {
          (httpService ? CreateFlowEventSource).mapTo[Source[ServerSentEvent, Any]]
        }
      }
    }
    ...
    def messageEvents = path("message-events") {
      ...
      get {
        complete {
          (httpService ? CreateMessageEventSource).mapTo[Source[ServerSentEvent, Any]]
        }
      }
    }
    // format: ON
...
class HttpService(interface: String, port: Int, selfTimeout: Timeout, flowFacade: ActorRef, flowFacadeTimeout: Timeout, mediator: ActorRef)
  ...
  protected def createFlowEventSource(): Source[ServerSentEvent, Any] = createEventSource(
    FlowEventPublisher.props(mediator, settings.flowEventPublisher.bufferSize)
  )

  protected def createMessageEventSource(): Source[ServerSentEvent, Any] = createEventSource(
